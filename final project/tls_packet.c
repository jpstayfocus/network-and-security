...
SSL/TLS packcet module aggragator
Takes the network packcet to create a sequence for the encryption handshake events
...
int findMatrix( char *sip, char *sport, char *dip, char *dport){
	...
	for(int k=0; k<matrixSize; k++){
		if((strcmp(matrix[k].dest,dip)==0 && strcmp(matrix[k].source,sip)==0) || (strcmp(matrix[k].dest,sip)==0 && strcmp(matrix[k].source,dip)==0)){ 
			//Only ip comparision because the port may change
			return k;
		}
	}
...
int main(int argc, char *argv[]){
	...
	while ((read = getline(&line, &len, fp)) != -1) {
	    if(strstr(line,target) != NULL) {
	    	...
	    	if(dataParse.numberTag >0){
				...
				for(int j=0; j<dataParse.numberTag; j++){
    ...
    for(int i=0; i < dataParseLogSize; i++){
    	for(int j=0; j<dataParseLog[i].numberTag; j++){
			...
			if(matrixID !=-1){
				...
				if(newTag == 1 || newTag ==2){
					//new session "hello_request" "client_hello"
